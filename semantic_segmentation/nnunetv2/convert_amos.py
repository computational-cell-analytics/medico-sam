...
def _write_dataset_json_file(trg_dir, dataset_name):
    ...
    data = {
        ...
        "file_ending": ".nii.gz",
        "name": dataset_name,
        "description": "AMOS: https://doi.org/10.48550/arXiv.2206.08023"
    }
...
def convert_amos_for_training(path, trg_dir, dataset_name):
    train_image_paths, train_gt_paths = _get_paths(path, "amos", "train")
    val_image_paths, val_gt_paths = _get_paths(path, "amos", "val")
...
def main():
    path = os.path.join(DATA_ROOT, "amos")
    dataset_name = "Dataset302_AMOS"

    # space to store your top-level nnUNet files
    trg_root = NNUNET_ROOT

    convert_amos_for_training(path=path, trg_dir=trg_root, dataset_name=dataset_name)
