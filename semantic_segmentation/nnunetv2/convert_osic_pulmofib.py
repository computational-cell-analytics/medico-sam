...
def _write_dataset_json_file(trg_dir, dataset_name):
    ...
    data = {
        ...
        "file_ending": ".nii.gz",
        "name": dataset_name,
        "description": "OSIC PulmoFib: https://www.kaggle.com/c/osic-pulmonary-fibrosis-progression/data"
    }
...
def convert_osic_pulmofib_for_training(path, trg_dir, dataset_name):
    train_image_paths, train_gt_paths = _get_paths(path, "osic_pulmofib", "train")
    val_image_paths, val_gt_paths = _get_paths(path, "osic_pulmofib", "val")
...
def convert_osic_pulmofib_for_testing(path, trg_dir, dataset_name):
    test_image_paths, test_gt_paths = _get_paths(path, "osic_pulmofib", "test")
...
def main():
    path = os.path.join(DATA_ROOT, "osic_pulmofib")
    dataset_name = "Dataset303_OSICPulmoFib"

    # space to store your top-level nnUNet files
    trg_root = NNUNET_ROOT

    # convert_osic_pulmofib_for_training(path=path, trg_dir=trg_root, dataset_name=dataset_name)
    # create_json_files(trg_dir=trg_root, dataset_name=dataset_name, write_json_function=_write_dataset_json_file)

    convert_osic_pulmofib_for_testing(path=path, trg_dir=trg_root, dataset_name=dataset_name)
