...
def _write_dataset_json_file(trg_dir, dataset_name):
    ...
    data = {
        ...
        "file_ending": ".tif",
        "name": dataset_name,
        "description": "PICCOLO: https://www.biobancovasco.bioef.eus/en/Sample-and-data-e-catalog/Databases/PD178-PICCOLO-EN1.html"  # noqa
    }
...
def convert_piccolo_for_training(path, trg_dir, dataset_name):
    train_image_paths, train_gt_paths = _get_paths(path, "piccolo", "train")
    val_image_paths, val_gt_paths = _get_paths(path, "piccolo", "val")
...
def convert_piccolo_for_testing(path, trg_dir, dataset_name):
    test_image_paths, test_gt_paths = _get_paths(path, "piccolo", "test")
...
def main():
    path = os.path.join(DATA_ROOT, "piccolo")
    dataset_name = "Dataset208_PICCOLO"

    # space to store your top-level nnUNet files
    trg_root = NNUNET_ROOT

    convert_piccolo_for_training(path=path, trg_dir=trg_root, dataset_name=dataset_name)
    create_json_files(trg_dir=trg_root, dataset_name=dataset_name, write_json_function=_write_dataset_json_file)

    convert_piccolo_for_testing(path=path, trg_dir=trg_root, dataset_name=dataset_name)
