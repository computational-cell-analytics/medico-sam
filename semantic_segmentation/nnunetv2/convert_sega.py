...
def _write_dataset_json_file(trg_dir, dataset_name):
    ...
    data = {
        ...
        "file_ending": ".nii.gz",
        "name": dataset_name,
        "description": "SegA: https://multicenteraorta.grand-challenge.org/"
    }
...
def convert_sega_for_training(path, trg_dir, dataset_name):
    train_image_paths, train_gt_paths = _get_paths(path, "sega", "train")
    val_image_paths, val_gt_paths = _get_paths(path, "sega", "val")
...
def convert_sega_for_testing(path, trg_dir, dataset_name):
    test_image_paths, test_gt_paths = _get_paths(path, "sega", "test")
...
def main():
    ...
    # convert_sega_for_training(path=path, trg_dir=trg_root, dataset_name=dataset_name)
    # create_json_files(trg_dir=trg_root, dataset_name=dataset_name, write_json_function=_write_dataset_json_file)

    convert_sega_for_testing(path=path, trg_dir=trg_root, dataset_name=dataset_name)
