...
def _interactive_seg_stats():
    ...
    for ei, exp in enumerate(exps):
        ...
        ax.bar(offsets, p_mean, width=bar_width, color=point_colors, label="point" if ei == 0 else "", zorder=1)
        ax.bar(
            offsets, delta_mean, width=bar_width, bottom=p_mean,
            label="box" if ei == 0 else "", zorder=1, color=box_colors
        )
    ...
    secax = ax.secondary_xaxis(-0.05, functions=(lambda x: x, lambda x: x))
    ...
    ax.set_title("Interactive Segmentation (Breast Tumor in Ultrasound)")
