...
def run_curvas_inference(output_channels):
    ...
    # Get the predictor and segmenter (checking if we can re-use micro-sam stuff)
    unetr = get_unetr(
    ...
    for image_path, gt_path in zip(image_paths, gt_paths):
        ...
        # Get predictions.
        outputs = []
        for per_slice in tqdm(image, desc="Predict per volume"):
            outputs.append(unetr(per_slice[None, None]).detach().cpu())
